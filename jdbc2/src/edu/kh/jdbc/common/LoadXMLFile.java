...
public class LoadXMLFile {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/* 왜 XML 파일을 이용해서 DB연결 정보를 읽어와야 될까?
			 * 
			 * 	1. 코드 중복 제거
			 * 	2. 별도 관리 용도 (관리의 용이성 향상) : 별도 파일을 이용해서 수정이 용이 
			 *  3. (중요) 재 컴파일을 진행하지 않기 위해서
			 *     - 코드가 길수록 컴파일에 소요되는 시간이 크다.
			 *     -> 코드 수정으로 인한 컴파일 소요시간을 없앰
 			 * 		  ( 파일의 내용을 읽어오는 코드만 작성해두면 java코드 수정 없이,
 			 *          파일 내용만 수정하면 재 컴파일은 수행되지 않는다.)	
			 *  4. XML파일에 작성된 문자열 현태를 그대로 읽어오기 때문에 SQL 작성 시 좀 더 편리해진다. 
			 *  
			 *  
			 *  
			 *  
